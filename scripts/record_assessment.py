...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Record explicit learner evidence for one Marinetime assessment and "
            "update concept ownership state."
        )
    )
    ...
    p.add_argument(
        "--evidence-type",
        required=True,
        choices=("retrieval", "oral", "scenario", "delayed_recall"),
    )
...
def main() -> int:
    ...
    try:
        ...
        before = {
            concept_id: {
                "ownership_stage": item.get("ownership_stage"),
        ...
        print(
            f"ASSESSMENT_RECORD_FAILED:{type(exc).__name__}:"
    ...
    for item in changed:
        print(
            ...
            f"mastery={float(item['from_score'] or 0.0):.3f}->"
            f"{float(item['to_score'] or 0.0):.3f}"
        )
